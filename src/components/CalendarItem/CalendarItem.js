...
const CalendarItem = ({ Item, today, events, openForm, filterMonth }) => {
  return (
    <div
      className={
      ...
      onClick={() => filterMonth(Item.format("x"))}
    >
      ...
      <div className="ItemTask">
        {events
          .filter(
            (event) =>
              event.date >= Item.format("x") &&
              event.date < Item.clone().add(1, "day").format("x")
          )
          .map((event) => (
            <div
              onClick={() => openForm("Update", event)}
              className="event"
              key={event.id}
            >
              <p>{event.title}</p>
