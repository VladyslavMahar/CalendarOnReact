...
const FormEvent = ({
  ...
  const addEvent = () => {
    if (updateEvent.title === "") alert("Title must be filled");
    else if (!updateEvent.date) alert("Date must be filled");
    else
      saveEvent(
        {
  ...
  const onChangeMethod = (value, input) => {
    setUpdateEvent((prevState) => ({ ...prevState, [input]: value }));
  };
  return (
    <div onClick={closeForm} className="formEventBlock">
      <form onClick={(e) => e.stopPropagation()} className="formEvent">
        ...
        <button type="button" onClick={addEvent} className="btnSave">
          Save
        </button>
      </form>
    </div>
